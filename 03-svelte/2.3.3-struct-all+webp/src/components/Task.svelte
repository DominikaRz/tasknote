    ...
    UIkit.util.on(`.js-lightbox${task.id}`, 'click', (e) => {
      ...
      UIkit.lightboxPanel({
        items: task.attachments.map(att => ({
          source: att.type === 'IMAGE' ? $fileCache.cachedFiles[att.path_compress] || `/webp/${att.path_compress}` : `/webp/${att.path_compress}`, ///srcMin/${att.path_compress_compress}
                            ...
                            {#each task.attachments as attach}
                              {#if attach.type == "IMAGE"}
                                  <div class="uk-cover-container">
                                    ...
                                    <img src={$fileCache.cachedFiles[attach.path_compress] || getPath(attach.path_compress)} alt={attach.path_compress} uk-cover />
                                  </div>
                              {/if}
                              ...
                              {#if attach.type == "FILE"}
                                  <div class="uk-cover-container">
                                    <canvas width="80" height="40"></canvas>
                                    <iframe src={$fileCache.cachedFiles[attach.path_compress] || getPath(attach.path_compress)} alt={attach.path_compress} uk-responsive uk-cover uk-video="automute: true"></iframe>
                                  </div>
                              {/if}
                              {#if attach.type == "VOICE"}
                                  <div class="uk-cover-container" style="border: black 2px solid">
                                    <canvas width="80" height="40"></canvas>
                                    <video src={$fileCache.cachedFiles[attach.path_compress] || getPath(attach.path_compress)} alt={attach.path_compress} uk-responsive uk-cover class="uk-position-center" uk-video="autoplay: false" poster="/img/play-button.svg" style="width: 2vw;"></video>
                                  </div>
                              {/if}
                            {/each}
                          </div>
