...
// @ts-nocheck
    ...
    onMount(() => {
      fileCache.loadFiles(attachments);
    });
  </script>
                  ...
                  <div class="uk-slider-items uk-child-width-1-2 uk-child-width-1-3@s uk-child-width-1-4@m">
                    {#each attachments  || [] as attachement}
                        {#if attachement.type == "IMAGE"}
                                <div>
                                    <canvas></canvas>
                                    <img  class="uk-blend-soft-light" src={$fileCache.cachedFiles[attachement.path_compress]|| getImagePath(attachement.path_compress)} alt="" uk-cover/> <!--src\{attachement.path_compress_compress}-->
