...
// @ts-nocheck
   ...
   import AttachmentModal from './AttachmentModal.svelte';
    ...
    const basePath = '/webp/'; // Base path for images in the public directory
  ...
  imageCache.loadImages(task.attachments);
 
    // Function to generate the appropriate lightbox items
  function generateLightboxItems(attachments) {
    return attachments.map(att => {
      if (att.type === 'IMAGE') {
        return { source: $imageCache.cachedImages[att.path_compress] || `/webp/${att.path_compress}`, caption: att.path_compress, type: 'image'};
                              ...
                              {#each task.attachments as attach}
                                ...
                                <div>
                                  <div class="uk-cover-container" >
                                    <canvas></canvas>
                                    <img src={$imageCache.cachedImages[attach.path_compress] || `/webp/${attach.path_compress}`} alt={attach.path_compress} 
                                    uk-cover/>
                                  </div>
                                </div>
                                {/if}
                                {#if attach.type == "FILE"}
                                  <div>
                                    <div class="uk-cover-container" >
                                      <canvas></canvas>
                                      <!-- svelte-ignore a11y-missing-attribute -->
                                      <iframe src='/webp/{attach.path_compress}' alt={attach.path_compress} uk-responsive  uk-cover
                                ...
                                {#if attach.type == "VOICE"}
                                  <div>
                                    <div class="uk-cover-container" style="border: black 2px solid" >
                                      ...
                                      <video src='/webp/{attach.path_compress}' alt={attach.path_compress} uk-responsive  uk-cover class="uk-position-center"
