...
// @ts-nocheck
  ...
  function generateLightboxItems(attachments) {
    return attachments.map(att => {
      if (att.type === 'IMAGE') {
        return { source: $fileCache.cachedFiles[att.path_compress] || `/webp/${att.path_compress}`, caption: att.path_compress, type: 'image'};
                              ...
                              {#each task.attachments as attach}
                                ...
                                <div>
                                  <div class="uk-cover-container" >
                                    <canvas></canvas>
                                    <img src={$fileCache.cachedFiles[attach.path_compress] || `/webp/${attach.path_compress}`} alt={attach.path_compress} 
                                    uk-cover/>
                                  </div>
                                </div>
                                {/if}
