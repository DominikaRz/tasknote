    ...
    UIkit.util.on(`.js-lightbox${task.id}`, 'click', (e) => {
      ...
      UIkit.lightboxPanel({
        items: task.attachments.map(att => ({
          source: att.type === 'IMAGE' ? $imageCache.cachedImages[att.path_compress] || `/webp/${att.path_compress}` : `/webp/${att.path_compress}`, ///srcMin/${att.path_compress_compress}
                            ...
                            {#each task.attachments as attach}
                              {#if attach.type == "IMAGE"}
                                  <div class="uk-cover-container">
                                    ...
                                    <img src={$imageCache.cachedImages[attach.path_compress] || getPath(attach.path_compress)} alt={attach.path_compress} uk-cover />
                                  </div>
                              {/if}
