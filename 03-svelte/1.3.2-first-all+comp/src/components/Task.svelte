...
// @ts-nocheck
  ...
  function generateLightboxItems(attachments) {
    return attachments.map(att => {
      if (att.type === 'IMAGE') {
        return { source: $fileCache.cachedFiles[att.path] || `/compress/${att.path}`, caption: att.path, type: 'image'};
                              ...
                              {#each task.attachments as attach}
                                ...
                                <div>
                                  <div class="uk-cover-container" >
                                    <canvas></canvas>
                                    <img src={$fileCache.cachedFiles[attach.path] || `/compress/${attach.path}`} alt={attach.path} 
                                    uk-cover/>
                                  </div>
                                </div>
                                {/if}
