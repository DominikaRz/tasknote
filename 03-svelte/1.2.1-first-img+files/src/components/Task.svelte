...
// @ts-nocheck
  ...
  function generateLightboxItems(attachments) {
    return attachments.map(att => {
      if (att.type === 'IMAGE') {
        return { source: $imageCache.cachedImages[att.path] || `/files/${att.path}`, caption: att.path, type: 'image'};
                              ...
                              {#each task.attachments as attach}
                                ...
                                <div>
                                  <div class="uk-cover-container" >
                                    <canvas></canvas>
                                    <img src={$imageCache.cachedImages[attach.path] || `/files/${attach.path}`} alt={attach.path} 
                                    uk-cover/>
                                  </div>
                                </div>
                                {/if}
                                {#if attach.type == "FILE"}
                                  <div>
                                    <div class="uk-cover-container" >
                                      <canvas></canvas>
                                      <!-- svelte-ignore a11y-missing-attribute -->
                                      <iframe src='/files/{attach.path}' alt={attach.path} uk-responsive  uk-cover
                                ...
                                {#if attach.type == "VOICE"}
                                  <div>
                                    <div class="uk-cover-container" style="border: black 2px solid" >
                                      ...
                                      <video src='/files/{attach.path}' alt={attach.path} uk-responsive  uk-cover class="uk-position-center"
