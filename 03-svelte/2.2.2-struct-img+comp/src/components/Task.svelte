    ...
    UIkit.util.on(`.js-lightbox${task.id}`, 'click', (e) => {
      ...
      UIkit.lightboxPanel({
        items: task.attachments.map(att => ({
          source: att.type === 'IMAGE' ? $imageCache.cachedImages[att.path] || `/compress/${att.path}` : `/compress/${att.path}`, ///srcMin/${att.path_compress}
          caption: att.path,
                            ...
                            {#each task.attachments as attach}
                              {#if attach.type == "IMAGE"}
                                  <div class="uk-cover-container">
                                    ...
                                    <img src={$imageCache.cachedImages[attach.path] || getPath(attach.path)} alt={attach.path} uk-cover />
                                  </div>
                              {/if}
                              ...
                              {#if attach.type == "FILE"}
                                  <div class="uk-cover-container">
                                    <canvas width="80" height="40"></canvas>
                                    <iframe src={getPath(attach.path)} alt={attach.path} uk-responsive uk-cover uk-video="automute: true"></iframe>
                                  </div>
                              {/if}
                              {#if attach.type == "VOICE"}
                                  <div class="uk-cover-container" style="border: black 2px solid">
                                    <canvas width="80" height="40"></canvas>
                                    <video src={getPath(attach.path)} alt={attach.path} uk-responsive uk-cover class="uk-position-center" uk-video="autoplay: false" poster="/img/play-button.svg" style="width: 2vw;"></video>
                                  </div>
                              {/if}
                            {/each}
                          </div>
