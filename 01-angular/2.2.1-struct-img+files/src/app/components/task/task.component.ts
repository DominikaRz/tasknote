...
export class TaskComponent implements OnInit, AfterViewInit {
  ...
  generateLightboxItems(attachments: Attachment[]): any[] {
    return attachments.map(att => {
      if (att.type === 'IMAGE') return { source: `/files/${att.path}`, caption: att.path, type: 'image' };
      if (att.type === 'FILE') return { source: `/files/${att.path}`, caption: att.path, type: 'iframe' };
      if (att.type === 'VOICE') return { source: `/files/${att.path}`, caption: att.path, type: 'video', attributes: { autoplay: false } };
      return null;
