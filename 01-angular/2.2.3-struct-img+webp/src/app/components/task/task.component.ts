...
export class TaskComponent implements OnInit, AfterViewInit {
    ...
    if (this.task && this.task.attachments) {
      this.task.attachments.forEach((attach: any) => {
        if (attach.type === 'IMAGE') {
          this.imageCacheService.cacheImage('/webp/' + attach.path_compress);
        }
      });
    }
  }
  ...
  generateLightboxItems(attachments: Attachment[]): any[] {
    return attachments.map(att => {
      if (att.type === 'IMAGE') return { source: `/webp/${att.path_compress}`, caption: att.path_compress, type: 'image' };
      if (att.type === 'FILE') return { source: `/webp/${att.path_compress}`, caption: att.path_compress, type: 'iframe' };
      if (att.type === 'VOICE') return { source: `/webp/${att.path_compress}`, caption: att.path_compress, type: 'video', attributes: { autoplay: false } };
      return null;
