...
function Attachement({ id, attachments }) {
  ...
  const imageUrls = attachments.map(attachment => constructPath(attachment.path_compress));
                 ...
                 hasImageType &&
                    ...
                    <div className="uk-width-expand uk-flex uk-flex-center" style={{backgroundColor: "transparent"}}>
                        ...
                        <div className="uk-position-relative uk-visible-toggle uk-light" tabIndex="-1">
                            ...
                            {attachments != null && attachments.map((attachment, index) =>  (
                                attachment.type == 'IMAGE' && (
                                <li key={attachment.id} className="slide">
                                  <div>
                                    <canvas></canvas>
                                    <img  className="uk-blend-soft-light" src={cachedFiles[constructPath(attachment.path_compress)] || constructPath(attachment.path_compress)} alt="" uk-cover="true"/>
                                    <div className="uk-position-center uk-panel">
                                          <a uk-icon="icon: trash; ratio: 1.3;"  data-id={attachment.id}
                                          onClick={() => handleDelete(attachments[index].id)} className='delete-att' ></a>
                                      </div>
                                  </div>
                                </li>
                                )
