...
const Task = ({ task }) => {
  ...
  const generateLightboxItems = (attachments, cachedImages) => {
    return attachments.map(att => {
      if (att.type === 'IMAGE') {
        return { source: cachedImages[constructPath(att.path)] || `/files/${att.path}`, caption: att.path, type: 'image' };
          ...
          <div className="uk-slider-items uk-child-width-1-2 uk-child-width-1-3@s uk-child-width-1-3@m">
            {task.attachments.map((attach, index) => (
              <div key={index} className="uk-cover-container">
                ...
                {attach.type === 'IMAGE' && (
                  <img src={cachedImages[constructPath(attach.path)] || `/files/${attach.path}`} alt={attach.path} uk-cover="true" />
                )}
                {attach.type === 'FILE' && (
                  <iframe src={`/files/${attach.path}`} alt={attach.path} uk-responsive="true" uk-cover="true" uk-video="automute: true"></iframe>
                )}
                {attach.type === 'VOICE' && (
                  <video type="audio/mpeg" src={`/files/${attach.path}`} alt={attach.path} uk-responsive="true" uk-cover="true" className="uk-position-center" poster="/img/play-button.svg" style={{ width: '2vw' }}></video>
                )}
              </div>
            ))}
          </div>
