...
const Task = ({ task }) => {
  ...
  const constructPath = (path) => `/webp/${path}`;

  const generateLightboxItems = (attachments, cachedFiles) => {
    return attachments.map(att => {
      if (att.type === 'IMAGE') {
        return { source: cachedFiles[constructPath(att.path_compress)] || `/webp/${att.path_compress}`, caption: att.path_compress, type: 'image' };
      } else if (att.type === 'FILE') {
        return { source: cachedFiles[constructPath(att.path_compress)] || `/webp/${att.path_compress}`, caption: att.path_compress, type: 'iframe' };
      } else if (att.type === 'VOICE') {
        return { source: cachedFiles[constructPath(att.path_compress)] || `/webp/${att.path_compress}`, caption: att.path_compress, type: 'video', attributes: { autoplay: false } };
      }
    });
  };
  
  const imageUrls = task.attachments.map(attachment => constructPath(attachment.path_compress));
  const { cachedFiles, isImagesLoaded } = useCache(imageUrls);


  useEffect(() => {
      const items = generateLightboxItems(task.attachments, cachedFiles);
          ...
          <div className="uk-slider-items uk-child-width-1-2 uk-child-width-1-3@s uk-child-width-1-3@m">
            {task.attachments.map((attach, index) => (
              <div key={index} className="uk-cover-container">
                ...
                {attach.type === 'IMAGE' && (
                  <img src={cachedFiles[constructPath(attach.path_compress)] || `/webp/${attach.path_compress}`} alt={attach.path_compress} uk-cover="true" />
                )}
                {attach.type === 'FILE' && (
                  <iframe src={cachedFiles[constructPath(attach.path_compress)] || `/webp/${attach.path_compress}`} alt={attach.path_compress} uk-responsive="true" uk-cover="true" uk-video="automute: true"></iframe>
                )}
                {attach.type === 'VOICE' && (
                  <video type="audio/mpeg" src={cachedFiles[constructPath(attach.path_compress)] || `/webp/${attach.path_compress}`} alt={attach.path_compress} uk-responsive="true" uk-cover="true" className="uk-position-center" poster="/img/play-button.svg" style={{ width: '2vw' }}></video>
                )}
              </div>
            ))}
          </div>
