...
const Task = ({ task }) => {
  ...
  const generateLightboxItems = (attachments, cachedImages) => {
    return attachments.map(att => {
      if (att.type === 'IMAGE') {
        return { source: cachedImages[constructPath(att.path_compress)] || `/webp/${att.path_compress}`, caption: att.path_compress, type: 'image' };
          ...
          <div className="uk-slider-items uk-child-width-1-2 uk-child-width-1-3@s uk-child-width-1-3@m">
            {task.attachments.map((attach, index) => (
              <div key={index} className="uk-cover-container">
                ...
                {attach.type === 'IMAGE' && (
                  <img src={cachedImages[constructPath(attach.path_compress)] || `/webp/${attach.path_compress}`} alt={attach.path_compress} uk-cover="true" />
                )}
                {attach.type === 'FILE' && (
                ...
                {attach.type === 'VOICE' && (
                  <video type="audio/mpeg" src={`/webp/${attach.path_compress}`} alt={attach.path_compress} uk-responsive="true" uk-cover="true" className="uk-position-center" poster="/img/play-button.svg" style={{ width: '2vw' }}></video>
                )}
              </div>
            ))}
          </div>
