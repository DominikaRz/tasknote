...
const Task = ({ task }) => {
  ...
  const generateLightboxItems = (attachments, cachedFiles) => {
    return attachments.map(att => {
      if (att.type === 'IMAGE') {
        return { source: cachedFiles[constructPath(att.path)] || `/compress/${att.path}`, caption: att.path, type: 'image' };
      } else if (att.type === 'FILE') {
        return { source: cachedFiles[constructPath(att.path)] || `/compress/${att.path}`, caption: att.path, type: 'iframe' };
      } else if (att.type === 'VOICE') {
        return { source: cachedFiles[constructPath(att.path)] || `/compress/${att.path}`, caption: att.path, type: 'video', attributes: { autoplay: false } };
      }
    });
  };
          ...
          <div className="uk-slider-items uk-child-width-1-2 uk-child-width-1-3@s uk-child-width-1-3@m">
            {task.attachments.map((attach, index) => (
              <div key={index} className="uk-cover-container">
                ...
                {attach.type === 'IMAGE' && (
                  <img src={cachedFiles[constructPath(attach.path)] || `/compress/${attach.path}`} alt={attach.path} uk-cover="true" />
                )}
                {attach.type === 'FILE' && (
                  <iframe src={cachedFiles[constructPath(attach.path)] || `/compress/${attach.path}`} alt={attach.path} uk-responsive="true" uk-cover="true" uk-video="automute: true"></iframe>
                )}
                {attach.type === 'VOICE' && (
                  <video type="audio/mpeg" src={cachedFiles[constructPath(attach.path)] || `/compress/${attach.path}`} alt={attach.path} uk-responsive="true" uk-cover="true" className="uk-position-center" poster="/img/play-button.svg" style={{ width: '2vw' }}></video>
                )}
              </div>
            ))}
          </div>
